...
class WebRTCApp {
  ...
  private clearTranscriptBtn!: HTMLButtonElement;
  ...
  constructor() {
    ...
    // Initialize empty transcript state
    this.showEmptyTranscriptState();
  ...
  private setupDOMElements(): void {
    ...
    this.transcriptDisplay = document.getElementById("transcript-display") as HTMLElement;
  ...
  private addToTranscript(message: string, type: "user" | "bot"): void {
    ...
    const emptyState = this.transcriptDisplay.querySelector('.transcript-empty');
    if (emptyState) {
      emptyState.remove();
    }

    const entry = document.createElement("div");
    entry.classList.add("transcript-entry", type);

    const meta = document.createElement("div");
    meta.classList.add("transcript-meta");

    const author = document.createElement("span");
    author.classList.add("transcript-author", type);
    author.textContent = type === "user" ? "You" : "Assistant";

    const time = document.createElement("span");
    time.classList.add("transcript-time");
  ...
  private clearTranscript(): void {
    ...
    this.transcriptDisplay.innerHTML = '<div class="transcript-empty">No conversation yet. Start talking to see the transcript here.</div>';
  }

  private showEmptyTranscriptState(): void {
    if (!this.transcriptDisplay) return;
    
    if (this.transcriptDisplay.children.length === 0) {
      this.transcriptDisplay.innerHTML = '<div class="transcript-empty">No conversation yet. Start talking to see the transcript here.</div>';
    }
  }
